...
export default {
  ...
  methods: {
    ...
    handleImageError(fullUrl, originalMediaUrl, event) {
      ...
      // 尝试重新加载图片
      setTimeout(() => {
        console.log('尝试重新加载图片:', fullUrl);
        const img = new Image();
        img.onload = () => {
          console.log('图片重新加载成功:', fullUrl);
    ...
    checkLoginStatus() {
      ...
      api.get('/api/login-status')
        .then(response => {
          console.log('登录状态响应:', response.data);
          // 明确设置登录状态
          ...
          console.log('更新后的登录状态:', this.isLoggedIn);
          console.log('更新后的用户信息:', this.user);
          console.log('更新后的管理员状态:', this.isAdmin);
        })
        .catch(error => {
          ...
          this.user = null;
          this.isAdmin = false;
          console.log('错误情况下设置登录状态为false，管理员状态为false');
        });
    },

    deleteEntry(id) {
        console.log('===== 删除按钮事件开始 =====');
        console.log('删除按钮被点击，条目ID:', id);
        ...
        if (confirm('确定要删除这条记录吗？')) {
          ...
          console.log('删除请求URL (相对路径):', url);
          
          api.delete('/api' + url)
            .then(response => {
              console.log('删除请求成功，响应状态:', response.status);
              console.log('删除请求成功，响应数据:', response.data);
              alert('删除成功！');
              this.fetchTimelineEntries();
            ...
            .catch(error => {
              ...
              if (error.response) {
                ...
                alert('删除失败: ' + error.response.status + ' - ' + error.response.data.message);
              } else {
                alert('删除失败: ' + error.message);
              }
            })
            .finally(() => {
              console.log('===== 删除按钮事件结束 =====');
            });
        } else {
          console.log('用户取消删除');
          console.log('===== 删除按钮事件结束 =====');
    ...
    fetchComments(id) {
      api.get(`/api/timeline/${id}/comments`)
        .then(response => {
    ...
    addComment(id) {
      ...
      api.post(`/api/timeline/${id}/comments`, { content })
        .then(response => {
          if (!this.comments[id]) {
            this.comments[id] = [];
          }
