...
class CORSConfigManager:
    ...
    def log_config(self):
        """记录当前CORS配置"""
        print(f"🔒 CORS配置 ({self.environment} 环境):")
        print(f"   域名: {self.domain or '未设置'}")
        print(f"   允许的源: {', '.join(self.get_origins())}")
        print(f"   支持凭证: {self.config['supports_credentials']}")
        print(f"   缓存时间: {self.config['max_age']}秒")
...
def validate_cors_config() -> bool:
    ...
    try:
        ...
        if '*' in origins and len(origins) > 1:
            print("⚠️  CORS配置包含通配符和具体域名，可能存在安全风险")
        
        if is_development_environment():
            print("✅ 开发环境CORS配置验证通过")
        else:
            if '*' in origins:
                print("❌ 生产环境不应使用CORS通配符")
                return False
            print("✅ 生产环境CORS配置验证通过")
